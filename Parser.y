...
DefListAux :: { Val }
DefListAux : DatumSeq                { mkList $ reverse $1 }
             -- here is where we handle the (. #0#) case; note
             -- the use of DefDatum instead of Datum.
           | dot DefDatum            { $2 }
           | DatumSeq1 dot Datum     { mkImpropList (reverse $1) $3 }
...
ListAux :: { Val }
ListAux : DatumSeq                   { mkList $ reverse $1 }
        | DatumSeq dot Datum         { mkImpropList (reverse $1) $3 }
...
parseDatum :: String -> Either String Val
parseDatum = runAlex' parse

prefix :: Token -> Val -> Val
prefix tok v = mkList [Symbol (desugar tok), v]
  where
