...
$explicit_sign = [\+\-]
  ...
  -- autogenerated. Note that '#' is explicitly excluded at the end.
...
initializePState completeMap
  = PState { workingMap = M.empty
...
runAlex' (Alex f) input =
  let r = f AlexState { alex_pos = alexStartPos
                      ...
                      , alex_bytes = []
                        ...
                        -- to go.
                      , alex_ust = initializePState $ mapFromResult r
                      , alex_scd = 0
                      }
  in case r of
    Left msg    -> Left msg
...
parseComplexAux parseReal r (hd:tl) = LitComplex <$> real <*> imag
  where
    (pre, post) = breakOnSign tl
    -- what's up with this split? We want to make sure we don't screw up
    -- an input like +1+2i or -1+2i. Even if there's no sign, there's always
    -- at least one character before the _second_ sign, so this is sound.    
    (reals, imags) = (hd:pre, init post) -- drop trailing i, re-add hd

    real = parseReal r reals
    imag = case imags of
          -- input was exactly +i or -i (below is +1+i etc.)
      []  -> Right $ LitInteger 1
      [_] -> Right $ LitInteger 1 -- there is always at least a sign here, 
                                  -- but if there is _only_ a sign,
                                  -- then we have _+i or _-i.
      other -> parseReal r other
    
    breakOnSign "i" = ("", "i") -- this happens with +i or -i
    breakOnSign s@('+':cs) = ("", s)
    breakOnSign s@('-':cs) = ("", s)
    breakOnSign (c:cs) =
      let (pre, post) = breakOnSign cs
      in (c:pre, post)
...
alexErrorWithPos (AlexPn _ line col) s = 
